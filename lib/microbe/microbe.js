 ...
 */
module.exports.microbe = function(_public, _private, callbackurl, port, redisConfig, fbConfig){
	...
	*/
	...
	// OR that the url is prefixed with a valid (harcoded for now) auth address
	var security = function(){
		return function(req,res, next){
			if (req.session.passport.user || req.url.indexOf('/auth/google') == 0 || req.url.indexOf('/auth/') == 0 ) next();
			else res.end(null,401); 
		}
	};
	...
	app.configure(function(){
		...
		app.use(express.static(_public)); // <-- add public static content before security mware
		app.use(security());
		app.use(express.static(_private)); // <-- now requires user be serialises into the session cookie
	});
